...
function sortByOrder(a, b) {
	const result = (a.order_ || 100) - (b.order_ || 100);
	return result;
}
...
class RenderEngine {
	...
	render(river) {
		...
		streamKeys.filter(each => each.startsWith("html_")).forEach(each => {
			...
			const $element = $(html);
			const parentQuery = zHtml.parentQuery_ || "body";
		...
		streamKeys
			...
			.forEach(each => {
				...
				$("#buttons").append(`<input id="${id}" placeholder="${label}">`);
				$(`#${id}`).on("blur", function () {
		...
		streamKeys.filter(each => each.startsWith("txt_")).forEach(each => {
			...
			const textPane = new Db.oClasses.TextPane(textStream);
			textPane.render();
		});
		streamKeys.filter(each => each.startsWith("fnd_")).forEach(each => {
			const id = each;
			const selectionKey = "selection_" + each;
			const finder = new Db.oClasses.Finder(id, $("#panes"), river[each], river[selectionKey]);
			finder.render();
		});
	}
...
const cattcher = {
	cattch(adb) {
		adb.aTestClasses.push(RenderEngine);
		adb.oClasses.RenderEngine = RenderEngine;
		Db = adb;
	},
};
export default cattcher;
